...
- Metodo VIP (R$1.500): https://vendas.incubadoradeexpert.com.br/metodo-vip/
- Sprint do Metodo (R$3.000): https://vendas.incubadoradeexpert.com.br/sprint-do-metodo/
...
- Diagnostico Ferramentas (R$97): https://vendas.incubadoradeexpert.com.br/diagnostico-ferramentas/

(URL antiga `vendas-incubadora.vercel.app` continua funcionando — Vercel mantem os 2 dominios ativos no mesmo projeto)
...
| Corrigir CTA de VIP/Sprint (apontavam pro produto errado) + ligar Express/Diagnostico/Grupo/Individual aos checkouts Voomp dedicados criados 12/08 | Gestor de Infra Arcane | **Done (13/08)** — 7 de 7 ofertas fechando no checkout Voomp | 6 produtos Voomp confirmados fora de analise | Ver detalhe completo no LOG de 13/08 abaixo. Grupo/Individual so migraram de gateway (Hotmart→Voomp) apos aprovacao explicita da Karol (Nivel 3 Consult) |
| Criar contratos de prestacao de servico (um por oferta) + fluxo de assinatura pro closer | Companion | Done — 06-07/08 | Arsenal + ecossistema de ofertas fechados | 8 contratos em `business/juridico/contratos/` (Termos Gerais + 1 por oferta + termo de compra do Expert360 + aditivo continuacao pos-Sprint). SOP de assinatura via ZapSign em `business/processos/assinatura-contratos-zapsign.md` — fluxo manual pelo closer (nao automatizado, volume atual cabe no plano gratuito). Pendente: revisao de advogado, endereco da sede da CONTRATADA, confirmar parcelamento 12x da Individual |
...
- 14/09 — @companion: **Sprint do Método passou de 5 pra 6 semanas — Processo Autoral e Ferramentas viraram entregas separadas.** Decisão da Karol: muita informação pro cliente assimilar/validar numa semana só. Nova estrutura: S1 Imersão, S2 Persona+Promessa, S3 Processo Autoral, S4 Ferramentas, S5 Proposta+Roteiros, S6 Agente de IA+Handoff. Preço (R$3.000) e demais termos comerciais não mudaram. Atualizado em cascata: `00-documento-mestre-ofertas.md`, `sprint-do-metodo-briefing-copy.md`, página live (`site/sprint-do-metodo/index.html`, incl. grid CSS de 5→3 colunas pra caber os 6 cards, e `site/sprint-do-metodo/diagnostico/index.html`), `docs/knowledge/expert-business/produto/ecossistema-ofertas-jul2026.md`, `arsenal-vendas-closer.md`, contrato jurídico (`juridico/contratos/03-sprint-do-metodo.md` + HTML regerado via `gerar-pdf-contratos.py` — **PDF final ainda não reexportado, passo manual via Playwright pendente**), Vitrine da Área de Membros (`area-de-membros/site/js/data.js`), diagnóstico de kickoff (`sprint-do-metodo/materiais/`) e os briefings de design (`direcao-visual-sprint-do-metodo.md`, `briefing-manus-infografico.md` — nenhuma imagem foi gerada ainda a partir desse briefing, então não há asset visual pra refazer). Decisão completa em log-decisoes 14/09. Pendente: commit via Ops (essas mudanças ainda não foram commitadas) e reexportar o PDF do contrato quando for usar com cliente real.
- 13/08 — @gestor-infra: **Corrigido bug real nos CTAs de VIP/Sprint + Express/Diagnóstico ligados aos checkouts Voomp dedicados.** Karol avisou que os 6 produtos novos da Voomp (criados 12/08, ver Mission Log #30) saíram de "Produto em Análise" — confirmei via Playwright (título + tag "REVISADO" em cada checkout, sem menção a análise pendente) antes de mexer em qualquer link de pagamento real. **Bug achado:** os CTAs de Método VIP e Sprint do Método ainda apontavam pra links `pay.voompcreators.com.br/15514/offer/{código}` — ofertas dentro do produto do Expert360º (o problema que a missão #30 identificou e corrigiu na Voomp, mas as páginas nunca tinham sido atualizadas pro link certo). Corrigido pros checkouts dedicados: VIP → `/16364`, Sprint → `/16365` (4 ocorrências cada, `Edit replace_all` no href). **Método Express e Diagnóstico Ferramentas** (que ainda usavam CTA de WhatsApp) trocados pro checkout Voomp (`/16363` e `/16368`) seguindo SOP-019 — Diagnóstico já tinha o ícone flutuante de WhatsApp (11/08), só precisou trocar o CTA principal + pixel `Contact`→`InitiateCheckout`; Método Express não tinha nada disso, então implementei CSS+HTML+JS do ícone do zero (copiado 1:1 do padrão já usado nas outras 3 páginas) além da troca de CTA. Validado: `node -c` no JS extraído das 4 páginas (sem erro), Playwright mobile 390px no Método Express confirmando que o ícone e a barra mobile sobem juntos sem colidir, `curl` em produção nos 2 domínios (novo e `.vercel.app`) confirmando os 4 hrefs corretos e zero resíduo de `cta-wa`. **Grupo e Individual (atualização no mesmo dia):** flagado pra Karol que Grupo/Individual continuavam no Hotmart mesmo com produto Voomp dedicado pronto (mudar gateway de pagamento de item de R$5-10 mil é Nível 3 — Consult). Karol aprovou a troca. Executado: `href` trocado de `pay.hotmart.com/W98400892E?off=...` pra `pay.voompcreators.com.br/16366` (Grupo) e `/16367` (Individual) — 4 ocorrências cada, mesmo padrão das outras (`Edit replace_all`). Essas 2 páginas já usavam classe `cta-checkout` com pixel `InitiateCheckout` desde a criação (não eram CTA de WhatsApp), então só o link mudou — sem ícone flutuante novo (não faz parte do padrão dessas 2, que sempre foram checkout direto). Validado `node -c` + `curl` em produção nos 2 domínios, zero resíduo de `hotmart`. **As 7 ofertas do ecossistema agora fecham 100% no checkout Voomp.**
- 13/08 — @gestor-infra: **Domínio próprio `vendas.incubadoradeexpert.com.br` no ar (SOP-020) — concluído.** Karol notou que as 7 páginas de vendas ainda estavam no domínio `vendas-incubadora.vercel.app` e pediu pra migrar. `vercel domains add` no projeto (confirmado `pwd` + `.vercel/project.json`, REGRA-016), Karol criou o registro A no Registro.br, propagação + emissão de certificado HTTPS pela Vercel levaram poucos minutos. Validado: `/metodo-vip/` responde 200 com pixel Meta intacto (raiz `/` dá 404 em ambos os domínios — normal, não existe index.html na raiz, só nas 7 subpastas). Links desta seção atualizados pro domínio novo; URL antiga continua no ar em paralelo (Vercel serve os 2 domínios no mesmo projeto). Junto nesta sessão: mesma missão resolvida também pra Área de Membros (`membros.incubadoradeexpert.com.br`) — ver tracker `campanhas/area-de-membros/tracker.md`.
- 11/08 — @companion: **Pivô de operação — Karol assume a reativação pessoalmente, closer pausado.** Orientação do Clone Euriler: sem receita recorrente/previsibilidade agora, Karol deve tomar a frente da venda até virar processo validado e repetível (closer entra depois, não descartado). Ferramenta (Supabase + planilha + sync n8n + arsenal) está pronta e é permanente — o projeto sai de ATIVOS no cockpit e vira Operação Contínua, já que não há mais fase de construção pendente, só operação. Decisão completa em log-decisoes 11/08.
- 09/08 — @ops: **Garantia corrigida pra bater com o contrato real, após venda do Sprint revelar a inconsistência.** Página do Sprint prometia "30 dias de garantia" mas o contrato (Termos Gerais, Cláusula 9) só cobre 7 dias corridos de arrependimento (art. 49 CDC) antes do início + reembolso proporcional às etapas não entregues (menos 20% de custo administrativo) depois disso — risco de publicidade enganosa (CDC art. 37). Corrigido nas 4 páginas de "Prestação de Serviço" (Sprint, VIP, Grupo, Individual) pra "7 dias de garantia incondicional" + explicação do proporcional no FAQ. Expert360 é caso à parte — produto digital self-paced, checkout automatizado, contrato próprio (Termo de Compra, não Prestação de Serviço) — Karol decidiu manter 30 dias ali; contrato do Expert360 (`07-expert360-termos-de-compra.md`) ganhou cláusula 5.2 nova garantindo isso explicitamente (antes só tinha os 7 dias legais, sem cobrir a promessa da página). HTML do PDF do Expert360 regenerado a partir do `.md`; o PDF final ainda não foi re-exportado via Playwright.
- 07/08 — @companion: **3 decisões de negócio pendentes fechadas pela Karol** — parcelamento da Individual (12x R$1.034,23, batendo com o valor já usado nas páginas de venda), cashback do Diagnóstico Ferramentas (15 dias corridos), prazo de acesso do Expert360º (12 meses). Aproveitei pra fechar também a plataforma de hospedagem no Termo de Compra do Expert360 (Voomp Play — já era fato conhecido do projeto, não decisão nova). Só resta 1 pendência menor (prazo de liberação de acesso do Expert360, hoje com exemplo "24h úteis" entre colchetes) e a revisão de advogado — nenhuma delas bloqueia o commit.
- 07/08 — @companion: **Endereço da sede fechado nos 9 contratos** (Rio Negro, nº 375, Casa 12, CEP 79023-041, Campo Grande/MS — foro Campo Grande/MS), passado pela Karol. Também corrigido um bug no script gerador: a função que cortava o rodapé antigo dos `.md` procurava um marcador de texto que eu mesmo tinha removido numa edição anterior, o que ia duplicar o campo Local/Data no PDF final — pego antes de virar problema. Único colchete fixo que restava nos contratos está fechado; só sobram 3 decisões de negócio da Karol (parcelamento Individual, prazo cashback Diagnóstico, prazo de acesso Expert360) e a revisão de advogado.
- 07/08 — @ops: **Revisão de copy e estrutura nas 7 páginas de vendas** (`paginas-vendas/site/`). (1) Removida menção à marca "Auroq" na objeção "não sou boa com tecnologia" em 5 páginas (Sprint, VIP, Grupo/Individual/Expert360 via arsenal e briefings) — trocado "sistema de IA (Auroq)" por "sistema de IA" genérico, já que é framework da Arcane/Euriler, não do método da Karol. (2) Reordenada a seção de autoridade/foto ("Diploma é o que você estudou...") pra depois das objeções em todas as 7 páginas — antes vinha logo após a oferta, quebrando o fluxo Pergunta→Resposta (autoridade só pesa depois que o preço já foi visto). (3) Preço em destaque trocado pra mostrar o valor parcelado em 12x grande com o à vista pequeno embaixo em 5 páginas: Sprint (12x R$300 / R$3.000), VIP (12x R$150 / R$1.500), Expert360 (12x R$49,70 / R$497), Grupo (12x R$517,12 / R$5.000), Individual (12x R$1.034,23 / R$10.000) — Método Express e Diagnóstico Ferramentas mantidos só no pix à vista.
- 07/08 — @companion: **SOP do ZapSign corrigido após teste real da Karol.** Campo "Texto" do plano gratuito não fica em branco pro cliente preencher sozinho (some se sair sem digitar) — na prática é quem monta o envio (Karol/closer) que digita os dados em cima do PDF-modelo, direto no ZapSign, antes de mandar assinar. Aproveitando o teste, também reestruturei o bloco "CONTRATANTE" dos 8 contratos (`business/juridico/contratos/`): antes era tudo numa frase corrida (5 campos colados, impossível posicionar campo por campo), agora é um campo por linha (nome/CPF/endereço/e-mail/telefone) — e o mesmo pro Local/Data antes da assinatura. PDFs em `pdf/` regenerados.
- 07/08 — @companion: **Contratos formatados na identidade visual real da Incubadora** — script `business/juridico/contratos/scripts/gerar-pdf-contratos.py` funde cada contrato específico com o Anexo I e gera HTML estilizado (mesma paleta/fonte das páginas de venda: preto #0B0B0C, laranja #FF6B1A, fonte Sora); Playwright exporta pra PDF final em `business/juridico/contratos/pdf/` (capa, cláusulas com destaque, tabelas com cabeçalho escuro, Anexo I com divisória própria, rodapé numerado). SOP do ZapSign simplificado por causa disso: closer agora sobe o PDF-modelo pronto (não gera nada) e usa campos preenchíveis do ZapSign pro próprio cliente completar os dados na hora de assinar, em vez de editar `.md`/rodar conversão a cada venda.
- 07/08 — @companion: **SOP de assinatura via ZapSign criado** (`business/processos/assinatura-contratos-zapsign.md`). Decisão: fluxo manual pelo closer (escolhe o contrato certo, preenche dados, converte pra docx reaproveitando `converter-md-para-docx.py`, sobe no ZapSign) — não automação via n8n/API, porque o volume atual cabe nos 5 documentos gratuitos/mês do ZapSign e a escolha do contrato/conferência de dados exige julgamento antes de mandar peça jurídica pra assinatura. Automação (Gestor de Infra Arcane) fica marcada como próximo passo só se o plano gratuito passar a estourar com regularidade.
- 06/08 — @companion: **8 contratos de prestação de serviço criados** em `business/juridico/contratos/` — Termos Gerais (Anexo I) + 1 contrato por oferta (Método Express, VIP, Sprint, Grupo, Individual, Diagnóstico Ferramentas) + Termo de Compra do Expert360º (checkout automatizado, sem contato pessoal) + Aditivo de continuação pós-Sprint. CONTRATADA fixa: Gestão pra Tudo - Karoline Franzini de Carvalho Senna, CNPJ 38.431.977/0001-36 (mesma chave PIX do arsenal). Cláusulas de reembolso pós-início (proporcional às etapas não realizadas, menos 20% de custo administrativo) e multa por atraso (2% — teto do CDC — + 5 dias úteis de tolerância) preenchidas com padrão de mercado, a pedido da Karol. **Pendente: revisão por advogado antes de usar com cliente real** — não é peça jurídica validada, é ponto de partida estruturado. Falta também o endereço da sede da CONTRATADA (não estava em nenhum arquivo do projeto).
- 03/08 — @data-engineer (Dara): **Dropdown + formatação condicional na coluna Status configurados na planilha.** Script Apps Script (`scripts/setup-status-dropdown.gs`, rodado pela Karol direto no Extensões > Apps Script da planilha) aplica dropdown restrito aos 5 valores do check constraint (`a_reativar/em_conversa/follow_up_marcado/fechou/desistiu`, `setAllowInvalid(false)` pra evitar typo que quebraria o sync com o Supabase) e formatação condicional colorindo a linha inteira (A:R) conforme o status. Idempotente, reaplicável se precisar mudar cor ou aumentar o limite de linhas (hoje 5000). Testado e confirmado funcionando pela Karol.
