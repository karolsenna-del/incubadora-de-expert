...
- 2026-07-30 — @b3-chief: Karol pediu automação de sync Planilha (Sheets) ↔ Supabase pro CRM Reativação de Leads. Esclarecido: fora do kit da Fase 2 (que é só Z-API/compras/disparos/recovery, gate QG-B3-003) — não inventado como passo daqui. Redirecionada pro Gestor de Infra Arcane, que constrói usando o mesmo servidor n8n já provisionado na Fase 0. Fase 2 continua pausada, sem mudança de estado.
- 2026-07-30 — @gestor-infra: **Fix de infra no servidor n8n** (fora do escopo dos gates do Bootstrap 3, documentado aqui por afetar o servidor provisionado na Fase 0). Credencial OAuth2 do Google Sheets no n8n falhava porque o callback (`/rest/oauth2-credential/callback`) é construído a partir de `WEBHOOK_URL` (`webhook.karolsenna.com.br`), mas esse host só roda o processo `n8n-webhook` (comando `webhook`), que não implementa rotas `/rest/*` — só webhooks de produção. Adicionado `N8N_EDITOR_BASE_URL=https://n8n.karolsenna.com.br/` no bloco `environment` compartilhado (`&n8n_env`) do `docker-compose.yml`, separando a URL do editor/credenciais da URL de webhook. Backup salvo antes (`docker-compose.yml.bak-20260730160807`), containers `n8n`/`n8n-worker`/`n8n-webhook` recriados com `docker compose up -d`, validado no ar (HTTP 200) e credencial testada com sucesso. Nenhum dado ou workflow existente afetado — só variável de ambiente nova.
- 2026-08-03 — @companion: item `Cloudflare` no cofre `Claude` tinha só o token de API (`op://Claude/Cloudflare/token`, escopo Edit zone DNS), sem senha de login da conta. Karol criou senha nova de login e salvou no 1Password separada do token, evitando confundir os dois no futuro.
