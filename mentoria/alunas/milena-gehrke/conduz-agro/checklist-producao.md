...
*(Atualizado 24/08 — reflete o progresso da construção das ferramentas do Portfólio + correção do modelo de venda: consultiva por Sessão Estratégica + PIX parcelado, sem checkout/gateway)*

- **P1 (bloqueia lançamento):** 8 itens reais (2 itens removidos por não se aplicarem ao modelo de venda — checkout Voomp e taxa de juros) — 4 feitos (formulários testados, Cartão de Identidade aprovado), 1 em progresso (Ficha de Inscrição — falta Milena reimplantar o script), 3 pendentes: roteiro da Sessão Estratégica, testar fluxo completo, data de lançamento
- **P2 (importante, não bloqueia venda mas bloqueia entrega):** 24 itens — 3 feitos (Simulador de Conversas, Leitura do Produtor, Roteiro de Condução), 10 em progresso (Mapa do Caso, Círculo de Controle, Matriz de Responsabilidade, Mapa de Posicionamento, Mapa de Valor Percebido, Checklist de Evidências, Raio-X, Plano de Aplicação Diária, CRM Comercial, Central de Condução — todos com arquivo/formulário já criado, falta só Milena rodar/reimplantar o script), 11 pendentes (inclui gravar os áudios de mentalidade)
...
**Leitura prática:** o produto já pode ser vendido — o bloqueio real caiu de 6 pra 3 itens P1 com a correção do modelo de venda (sem checkout, sem página de vendas obrigatória). O que falta é o roteiro da Sessão Estratégica, testar o fluxo ponta a ponta e definir a data de lançamento. As ferramentas do Portfólio (Seção 1) estão avançando bem: metade já feita ou com arquivo pronto esperando só a Milena rodar o script no Apps Script.
