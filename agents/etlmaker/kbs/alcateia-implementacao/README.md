# KB — Alcateia Implementação
**Expert:** Vini Grevy  
**Produto:** Alcateia Implementação (vinigrevy.memberkit.com.br)  
**Versão KB:** 1.0  
**Produzida por:** ETLmaker v3.0  
**Data:** 2026-05-29  

---

## O que é esta KB

Esta Knowledge Base contém o conhecimento tratado do curso **Alcateia Implementação** de Vini Grevy — um sistema de implementação para experts que querem construir um negócio digital previsível a partir do seu conhecimento.

A KB foi extraída de 92 transcrições de aulas (de 94 disponíveis) e organizada em 7 volumes temáticos por um pipeline ETL completo.

---

## Para quem serve

Esta KB é útil para:
- Experts querendo replicar a estrutura da Alcateia no próprio negócio
- Consultores que precisam entender o método para adaptar ao cliente
- Agentes de IA que precisam de contexto do método Alcateia para responder perguntas

---

## Volumes Disponíveis

| Volume | Escopo | Prioridade de Leitura |
|--------|--------|----------------------|
| [VOL-fundamentos](VOL-fundamentos.md) | Curva J, modelo de negócio, 4 escalas, caminho 30k, caixa rápido | 1° |
| [VOL-posicionamento](VOL-posicionamento.md) | Mercado único, promessa matadora, método AB, perfil matador, Instagram Fantasma | 2° |
| [VOL-oferta](VOL-oferta.md) | Precificação, criação de método, níveis L1/L2/L3, mentoria/infoproduto | 3° |
| [VOL-conteudo](VOL-conteudo.md) | 3 tipos de conteúdo, linha editorial, teia de aranha, frequência | 4° |
| [VOL-funil-vendas](VOL-funil-vendas.md) | Cilindro de vendas, mini treinamento, aula milionária (29 atos), ManyChat, GreatPages, VTurb | 5° |
| [VOL-social-selling](VOL-social-selling.md) | Social selling Instagram+WhatsApp, protocolo Carol, CRM, downsell, reativação | 6° |
| [VOL-vendas-trafego](VOL-vendas-trafego.md) | Método Cedo, scripts, métricas, turbinar, escala flutuante, remarketing | 7° |

---

## Mapa do Método (visão geral)

```
FUNDAMENTOS
  Curva J → Caminho 30k → 4 Escalas → Problema como Ativo
          ↓
POSICIONAMENTO
  Mercado Único → Promessa Matadora → Método AB → Perfil Matador
          ↓
OFERTA
  Precificação → Níveis L1/L2/L3 → Template de Serviço
          ↓
CONTEÚDO
  3 Tipos → Linha Editorial → Teia de Aranha → Consistência
          ↓
FUNIL DE VENDAS
  Cilindro de Vendas
  ├── Mini Treinamento → WhatsApp → Mentoria R$1k-3k
  └── Aula Milionária (VSL) → Checkout → Infoproduto R$197-497
          ↓
VENDAS
  Social Selling (Instagram → WhatsApp)
  Método Cedo → Script de Fechamento → CRM → Reativação
          ↓
TRÁFEGO E ESCALA
  Turbinar → Métricas → Escala Flutuante → Remarketing
```

---

## Autores

| Autor | Contribuição |
|-------|-------------|
| **Vini Grevy** | Criador do método. Cobre todos os domínios. Resultado pessoal: 15M+ faturado, 6.500+ alunos, 400+ mentorados |
| **Carol** | Especialista em social selling da Alcateia. Fecha ~99% das vendas. Criou os 3 princípios e o protocolo WhatsApp |

---

## Limitações desta KB

- Aulas 90 e 93 são PDFs — não transcritas. Podem conter artefatos visuais (rotinas de stories) não cobertos
- Aulas 58–68 são tutoriais de configuração de plataforma (GreatPages, VTurb) — não indexadas por conteúdo operacional sem valor metodológico
- Métricas numéricas (preços, taxas de conversão) refletem o contexto de Vini Grevy em 2024 — usar como referência de magnitude
- Scripts de vendas são templates — devem ser adaptados ao nicho e voz de cada expert

---

## Documentação do Pipeline

- [MAPA-TERRITORIAL.md](00-pipeline/MAPA-TERRITORIAL.md) — mapeamento completo dos 11 domínios
- [REGRAS-CARDINAIS.md](REGRAS-CARDINAIS.md) — todas as regras invioláveis do método
- [REPERTORIO.md](REPERTORIO.md) — frameworks e fórmulas prontos para uso
- [GLOSSARIO.md](GLOSSARIO.md) — glossário consolidado de todos os volumes
